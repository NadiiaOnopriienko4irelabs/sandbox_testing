...
async function getTransactions(startBlock, blockHash, accountId) {
 try {
     const near = await connect(exports.config);
     const blockArr = [];
     do {
         console.log('start function tx_detail')
         const currentBlock = await getBlockByID(blockHash);
         console.log(' current blok')
     ...
     return { status: 'success', result: { matchingTransactions: transactions, transactionsLinks: txsLinks } };
